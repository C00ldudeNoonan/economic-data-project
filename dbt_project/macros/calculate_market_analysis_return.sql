{% macro calculate_market_analysis_return(source_ref) %}
WITH daily_data AS (
    SELECT
        symbol,
        exchange,
...
daily_changes AS (
    SELECT
        ...
        adj_volume,
        prev_adj_close,
        adj_close - prev_adj_close AS daily_diff
    FROM daily_data
),
...
price_lookups AS (
    SELECT
        ...
        dc.exchange,
        dc.date,
        dc.adj_close AS current_price,
        dc.adj_high AS current_high,
        dc.adj_low AS current_low,
        dc.volume AS current_volume,
        dc.daily_diff,
        p1.adj_close AS price_30d_ago,
        p2.adj_close AS price_90d_ago,
        p3.adj_close AS price_180d_ago,
        p4.adj_close AS price_270d_ago,
        p5.adj_close AS price_365d_ago
