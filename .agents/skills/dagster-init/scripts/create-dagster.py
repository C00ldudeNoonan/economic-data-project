...
def get_project_name(provided_name=None):
    ...
    while True:
        name = input(
            "\nEnter project name (or press Enter for 'my-dagster-project'): "
        ).strip()
...
def create_project_with_uvx(project_name):
    ...
    try:
        ...
        if result.returncode != 0:
            print("L Failed to create project")
...
def display_success_message(project_name):
    ...
    print("   2. uv run dg dev")
    print("   3. Open http://localhost:3000 in your browser")

    print("\n==> Useful commands:")
    print("   - uv run dg check defs  - Validate your definitions")
    print("   - uv run pytest         - Run tests")
    print("   - uv add <package>      - Add new dependencies")
    print("   - uv run python         - Run Python in the venv")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create a new Dagster project with uv")
    parser.add_argument(
        "project_name", nargs="?", help="Name of the Dagster project to create"
    )
