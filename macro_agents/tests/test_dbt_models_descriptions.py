    ...
    for sql_file in sql_files:
        ...
        if not schema_file.exists():
            ...
            if parent_schema.exists():
                ...
                models_without_descriptions.append(
                    f"{sql_file.relative_to(dbt_project_path)} (no schema.yml found)"
                )
                continue
        ...
        try:
            ...
            models_without_descriptions.append(
                f"{sql_file.relative_to(dbt_project_path)} (error reading schema.yml: {e})"
            )
            continue
        ...
        if not model_found:
            models_without_descriptions.append(
                f"{sql_file.relative_to(dbt_project_path)} (not found in schema.yml)"
            )
        elif not model_has_description:
            models_without_descriptions.append(
                f"{sql_file.relative_to(dbt_project_path)} (missing description)"
            )
