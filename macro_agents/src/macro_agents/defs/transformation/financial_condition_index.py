...
from macro_agents.defs.transformation.dbt import full_dbt_assets  # Import the dbt assets
...
def fetch_financial_data(motherduck_resource: MotherDuckResource) -> pl.DataFrame:
    ...
    if len(bond_data) > 0:
        bond_processed = (
            ...
            .with_columns([
                ...
                pl.when(pl.col("value") == ".").then(None).otherwise(pl.col("value")).cast(pl.Float64).alias("value")  # Handle "." values
            ])
            .sort("date")
            .with_columns([
                pl.col("value").rolling_mean(window_size=90, min_periods=1).alias("90_day_avg")
            ])
            .with_columns([
                pl.col("90_day_avg").shift(3).alias("prev_90_day_avg")
            ])
            .with_columns([
    ...
    if len(dow_data) > 0:
        dow_processed = (
            ...
            .sort("date")
            .with_columns([
                pl.col("value").shift(3).alias("prev_value")
            ])
            .with_columns([
    ...
    if len(mortgage_data) > 0:
        mortgage_processed = (
            ...
            .with_columns([
                pl.col("90_day_avg").shift(1).alias("prev_90_day_avg")
            ])
            .with_columns([
    ...
    if len(housing_data) > 0:
        housing_processed = (
            ...
            .with_columns([
                ((pl.col("value") / pl.col("prev_value")).log() * 100).alias("housing")
            ])
            .select(["date", "housing"])
            .filter(pl.col("date") >= pl.date(2014, 4, 30))
        )
    ...
    if len(dollar_data) > 0:
        dollar_processed = (
            ...
            .with_columns([
                ((pl.col("90_day_avg") / pl.col("prev_90_day_avg")).log()).alias("dollar")
            ])
            .with_columns([
                pl.col("dollar").fill_null(0)
            ])
            .select(["date", "dollar"])
            .filter(pl.col("date") >= pl.date(2014, 4, 30))
        )
...
def calculate_fci_scores(merged_df: pl.DataFrame, weights: Dict[str, List[float]]) -> pl.DataFrame:
    """
        ...
        if component in merged_df.columns:
            ...
            for i in range(len(merged_df)):
                if i < window_size - 1:
                    ...
                    window_values = merged_df[component][i-window_size+1:i+1].to_list()
                    
                    # Filter out None values and get corresponding weights
                    valid_indices = [j for j, val in enumerate(window_values) if val is not None]
                    ...
                    if len(valid_values) > 0 and len(valid_values) == len(valid_weights):
    ...
    score_columns = [f"{comp}_score" for comp in components if f"{comp}_score" in result_df.columns]
...
@dg.asset(
    ...
    tags={"schedule": "daily", "execution_time": "weekdays_6am_est", "analysis_type": "financial_index"},
)
def financial_conditions_index(
    ...
    try:
        ...
        if merged_df is None or len(merged_df) == 0:
            raise ValueError("No data available for FCI calculation. Make sure stg_fred_series is materialized.")
        
        context.log.info(f"Successfully fetched data with {len(merged_df)} records")
        context.log.info(f"Date range: {merged_df['date'].min()} to {merged_df['date'].max()}")
        ...
        context.log.info(f"FCI Statistics: Min={stats.get('min_fci', 'N/A'):.4f}, "
        ...
        return dg.MaterializeResult(
            metadata={
                ...
                "latest_fci_value": float(latest_value) if len(latest_fci) > 0 else None,
...
def fci_weights_config(
    ...
    context.log.info(f"FCI weights configuration saved with {len(weights_df)} weight periods")
