...
class EconomyStateAnalysisSignature(dspy.Signature):
    ...
    analysis: str = dspy.OutputField(
        desc="""Comprehensive economic state analysis including:
        ...
        Focus on quantitative metrics, trends, and leading indicators including commodity markets to provide a clear assessment of the current economic state."""
    )
...
class EconomyStateModule(dspy.Module):
    ...
    def forward(self, economic_data: str, commodity_data: str):
        return self.analyze_state(
            economic_data=economic_data, commodity_data=commodity_data
        )
...
class EconomicAnalysisResource(dg.ConfigurableResource):
    ...
    def get_commodity_data(self, md_resource: MotherDuckResource) -> str:
    ...
    def get_correlation_data(
        ...
        if hasattr(md_resource, "query_sampled_data"):
            ...
            query = f"""
            SELECT *
            FROM leading_econ_return_indicator
            ORDER BY GREATEST(
...
def analyze_economy_state(
    ...
    """
    context.log.info("Starting economy state analysis...")
    ...
    context.log.info(
        "Running economy state analysis with economic and commodity data..."
    )
