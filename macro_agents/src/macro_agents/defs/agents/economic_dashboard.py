...
def economic_dashboard(
    ...
    dashboard_summary = {
        ...
        "data_freshness": {
            ...
            "asset_allocation": dashboard_data.get("asset_allocation", {}).get(
                "timestamp"
            ),
            "integrated_analysis": dashboard_data.get("integrated_analysis", {}).get(
                "timestamp"
            ),
        },
        "schedule_type": "daily",
...
def economic_monitoring_alerts(
    ...
    monitoring_summary = {
        ...
        "alert_breakdown": {
            "economic_indicators": len(
                [a for a in alerts if a["type"] == "economic_indicator"]
            ),
            "market_performance": len(
                [a for a in alerts if a["type"] == "market_performance"]
            ),
        },
