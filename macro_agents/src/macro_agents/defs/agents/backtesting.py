...
class PerformanceCalculator:
    ...
    def calculate_returns(
        price_data: pl.DataFrame, start_date: str, end_date: str
    ) -> Dict[str, float]:
        ...
        for symbol in price_data["symbol"].unique():
            ...
            start_price = (
                symbol_data.filter(pl.col("date") <= start_date)
                .sort("date", descending=True)
                .select("close")
                .head(1)
            )
            end_price = (
                symbol_data.filter(pl.col("date") <= end_date)
                .sort("date", descending=True)
                .select("close")
                .head(1)
            )
    ...
    def calculate_metrics(
        returns: List[float], benchmark_returns: Optional[List[float]] = None
    ) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "max_drawdown": PerformanceCalculator._calculate_max_drawdown(
                returns_array
            ),
        ...
        if benchmark_returns:
            ...
            metrics["beta"] = (
                np.cov(returns_array, benchmark_array)[0, 1] / np.var(benchmark_array)
                if np.var(benchmark_array) > 0
                else 0
            )
            metrics["information_ratio"] = (
                metrics["alpha"] / np.std(returns_array - benchmark_array)
...
class BacktestingEngine(dg.ConfigurableResource):
    ...
    def get_historical_analysis(
        self, md_resource: MotherDuckResource, as_of_date: str
    ...
    def get_historical_prices(
        ...
        return md_resource.execute_query(
            query,
            read_only=True,
            params=[
                start_date,
    ...
    def run_backtest(
        ...
        allocation_analysis = self.get_asset_allocation_analysis(
            md_resource, prediction_date_str
        )
        if not allocation_analysis:
            raise ValueError(
                f"No asset allocation analysis found for date {prediction_date_str}"
            )
        ...
        actual_returns = calculator.calculate_returns(
            price_data, prediction_date_str, end_date
        )
        ...
        prediction_accuracy = self._calculate_prediction_accuracy(
            predicted_assets, actual_returns
        )
        ...
        if context:
            context.log.info(
                f"Backtest completed: {len(actual_returns)} assets analyzed"
            )

        return result

    def _calculate_prediction_accuracy(
        self, predicted_assets: List[Dict[str, Any]], actual_returns: Dict[str, float]
    ) -> Dict[str, float]:
        ...
        overweights = [
            ...
            if asset["action"] in ["OVERWEIGHT", "ALLOCATE"]
        ]
        underweights = [
            asset for asset in predicted_assets if asset["action"] == "UNDERWEIGHT"
        ]
        ...
        if overweights:
            ...
            overweight_returns = [
                actual_returns.get(symbol, 0)
                for symbol in overweight_symbols
                if symbol in actual_returns
            ]

            if overweight_returns:
                accuracy_metrics["overweight_accuracy"] = sum(
                    1 for r in overweight_returns if r > 0
                ) / len(overweight_returns)
                accuracy_metrics["overweight_avg_return"] = np.mean(overweight_returns)
                accuracy_metrics["overweight_positive_count"] = sum(
                    1 for r in overweight_returns if r > 0
                )
        ...
        if underweights:
            ...
            if underweight_returns:
                accuracy_metrics["underweight_accuracy"] = sum(
                    1 for r in underweight_returns if r < 0
                ) / len(underweight_returns)
                accuracy_metrics["underweight_avg_return"] = np.mean(
                    underweight_returns
                )
                accuracy_metrics["underweight_negative_count"] = sum(
                    1 for r in underweight_returns if r < 0
                )
        ...
        all_predicted_returns = [
            actual_returns.get(symbol, 0)
            for symbol in all_predicted_symbols
            if symbol in actual_returns
        ]
        ...
        if all_predicted_returns:
            ...
            for asset in predicted_assets:
                ...
                if symbol in actual_returns:
                    ...
                    if (
                        asset["action"] in ["OVERWEIGHT", "ALLOCATE"]
                        and actual_return > 0
                    ):
            ...
            accuracy_metrics["overall_accuracy"] = correct_predictions / len(
                all_predicted_returns
            )
    ...
    def write_backtest_results(
        ...
        result_record = {
            ...
            "backtest_timestamp": backtest_result.backtest_metadata[
                "backtest_timestamp"
            ],
        }
...
def backtest_agent_predictions(
    ...
    result_metadata = {
        ...
        "prediction_date": backtest_result.prediction_date,
        "prediction_horizon_days": backtest_result.prediction_horizon_days,
        "total_assets_analyzed": backtest_result.backtest_metadata[
            "total_assets_analyzed"
        ],
        "assets_with_data": backtest_result.backtest_metadata["assets_with_data"],
        "overall_accuracy": backtest_result.prediction_accuracy.get(
            "overall_accuracy", 0
        ),
        "overweight_accuracy": backtest_result.prediction_accuracy.get(
            "overweight_accuracy", 0
        ),
        "underweight_accuracy": backtest_result.prediction_accuracy.get(
            "underweight_accuracy", 0
        ),
...
def batch_backtest_analysis(
    ...
    context.log.info(
        f"Found {len(prediction_dates)} prediction dates for batch backtesting"
    )
    ...
    if prediction_dates:
        ...
        test_allocation = backtesting_engine.get_asset_allocation_analysis(
            md, test_date
        )
        ...
        if test_allocation:
            ...
            # Test extraction
            extractor = PredictionExtractor()
            test_recommendations = extractor.extract_asset_recommendations(
                test_allocation
            )
    ...
    if not validation_df.is_empty():
        row = validation_df[0]
        context.log.info(
            f"Data availability - Cycle: {row[0]}, Allocation: {row[1]}, Sectors: {row[2]}, Indices: {row[3]}"
        )

        if row[0] == 0 or row[1] == 0:
            raise ValueError(
                f"Missing required analysis data - Cycle: {row[0]}, Allocation: {row[1]}"
            )
    ...
    all_accuracies = [
        result.prediction_accuracy.get("overall_accuracy", 0)
        for result in batch_results
    ]
    all_returns = [
        result.performance_metrics.get("total_return", 0) for result in batch_results
    ]
    all_sharpe_ratios = [
        result.performance_metrics.get("sharpe_ratio", 0) for result in batch_results
    ]
    ...
    result_metadata = {
        ...
        "accuracy_consistency": 1
        - aggregate_metrics["accuracy_std"],  # Higher is more consistent
