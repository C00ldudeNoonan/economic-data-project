...
class BacktestingVisualizer(dg.ConfigurableResource):
    ...
    def _calculate_accuracy_summary(self, accuracy_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_overweight_accuracy": np.mean(overweight_accuracies) if overweight_accuracies else 0,
            "avg_underweight_accuracy": np.mean(underweight_accuracies) if underweight_accuracies else 0,
    ...
    def _generate_recommendations(self, performance_data: List[Dict[str, Any]], 
        ...
        avg_accuracy = np.mean([d["overall_accuracy"] for d in accuracy_data]) if accuracy_data else 0
        
        if avg_return < 0:
            recommendations.append("Average returns are negative - consider reviewing asset selection criteria")
        elif avg_return < 0.02:
            recommendations.append("Returns are low - consider optimizing for higher-return opportunities")
        
        if avg_accuracy < 0.5:
            recommendations.append("Prediction accuracy is below 50% - model needs significant improvement")
        elif avg_accuracy < 0.6:
            recommendations.append("Prediction accuracy is moderate - consider prompt optimization")
        
        # Volatility analysis
        avg_volatility = np.mean([d["volatility"] for d in performance_data])
        if avg_volatility > 0.3:
            recommendations.append("High volatility detected - consider risk management improvements")
        
        # Consistency analysis
        returns_std = np.std([d["total_return"] for d in performance_data])
        if returns_std > 0.1:
            recommendations.append("High return variability - consider more consistent prediction strategies")
    ...
    def create_model_evaluation_report(self, md_resource: MotherDuckResource,
                                     context: Optional[dg.AssetExecutionContext] = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            "model_health_score": self._calculate_model_health_score(evaluation_history),
            "improvement_trend": self._analyze_improvement_trend(evaluation_history),
            "recommendations": self._generate_model_recommendations(evaluation_history, optimization_history)
        }
    ...
    def _calculate_model_health_score(self, evaluation_history: List[Dict[str, Any]]) -> float:
    ...
    def _analyze_improvement_trend(self, evaluation_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyze improvement trend over time."""
        if len(evaluation_history) < 2:
            return {"trend": "insufficient_data", "change": 0.0}
        
        recent_scores = [eval_data.get("overall_score", 0) for eval_data in evaluation_history[:3]]
        older_scores = [eval_data.get("overall_score", 0) for eval_data in evaluation_history[3:6]]
    ...
    def _generate_model_recommendations(self, evaluation_history: List[Dict[str, Any]], 
                                      optimization_history: List[Dict[str, Any]]) -> List[str]:
        ...
        if health_score < 0.5:
            recommendations.append("Model health is poor - immediate optimization required")
        elif health_score < 0.7:
            recommendations.append("Model health is moderate - consider prompt optimization")
        
        # Check specific metrics
        if latest_eval.get("directional_accuracy", 0) < 0.6:
            recommendations.append("Directional accuracy is low - review economic indicator selection")
        
        if latest_eval.get("return_accuracy", 0) < 0.4:
            recommendations.append("Return prediction accuracy is low - improve return estimation methods")
        ...
        if len(optimization_history) == 0:
            recommendations.append("No optimization history - consider running initial optimization")
        elif len(optimization_history) < len(evaluation_history) / 2:
            recommendations.append("Infrequent optimization - consider more regular optimization cycles")
        
        if not recommendations:
            recommendations.append("Model performance is satisfactory - continue current approach")
...
def backtesting_performance_dashboard(
    ...
    dashboard_data = visualizer.create_performance_dashboard(md_resource=md, context=context)
    
    # Create model evaluation report
    evaluation_report = visualizer.create_model_evaluation_report(md_resource=md, context=context)
